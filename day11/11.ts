    ...
    for (let j = 0; j < row.length; j++) {
      ...
      if (row[j] === "L") {
        ...
        if (j % rowLength !== 0) {
          ...
          if (upperLeft >= 0) {adjacentSeatLocations.push(upperLeft);}
          if (lowerLeft < totalLength) {adjacentSeatLocations.push(lowerLeft);}
          if (flatItemIndex - 1 >= 0) {adjacentSeatLocations.push(flatItemIndex - 1);}
        }
        ...
        if (flatItemIndex % rowLength === 0 || j % (rowLength - 1) !== 0) {
          ...
          if (upperRight >= 0) {adjacentSeatLocations.push(upperRight);}
          if (lowerRight < totalLength) {adjacentSeatLocations.push(lowerRight);}
          if (flatItemIndex + 1 < totalLength) {adjacentSeatLocations.push(flatItemIndex + 1);}
        }
...
console.log(`Part 2: ${seatingSystem2(testData)}`); // 
